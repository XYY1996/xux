...
import com.xux.generator.freemarker.framework.model.TableInfo;
import com.xux.generator.freemarker.framework.model.ColumnInfo;
import com.xux.generator.freemarker.framework.util.DbUtil;
import com.xux.generator.freemarker.framework.util.FileHelper;
...
public class InitTask  extends AbstractApplicationTask {
    ...
    protected boolean doInternal(ApplicationContext context) throws Exception {
        ...
        if (Configuration.getString("base.database").equals(Constants.DB_ORACLE)) {
            PropertyUtil.loadProp(Global.getResourcePath("columnType2PropType_oracle.properties"));
        } else {
            PropertyUtil.loadProp(Global.getResourcePath("columnType2PropType.properties"));
        }

        // 属性类型与包类名的映射
        PropertyUtil.loadProp(Global.getResourcePath("propType2Package.properties"));

        // 属性类型与jdbc类型的映射，注意这里为了防止与上面冲突，属性类型前加了_
        PropertyUtil.loadProp(Global.getResourcePath("propType2JdbcType.properties"));
