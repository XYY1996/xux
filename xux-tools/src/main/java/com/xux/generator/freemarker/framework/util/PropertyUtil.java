...
public class PropertyUtil {
    ...
    public static void loadPropWithoutLog(String fileName) {
        ...
        // 首先在classpath中找，如果找不到，则在工作目录下找
        is = PropertyUtil.class.getClassLoader().getResourceAsStream(fileName);
        log.error("在classpath下找{0},是否找到：{1}", fileName, is == null ? "否" : "是");
        if (is == null) {
            // 没找到，则在使用绝对路径，找到jar所在的根目录
            // String rootPath = System.getProperty("user.dir");
            String rootPath = System.getProperty("logDirPath");
            log.error("rootPath:{0}", rootPath);
            ...
            try {
                is = new FileInputStream(rootPath + File.separator + fileName);
                reader = new InputStreamReader(is, "UTF-8");
                log.error("在classpath下没找到，在用户工作目录下找{0},是否找到：{1}", fileName, is == null ? "否" : "是");
            } catch (FileNotFoundException e) {
                e.printStackTrace();
                log.error("获取输入流异常:{0}", e);
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
                log.error("不支持的编码异常:{0}", e);
            }
        } else {
            try {
                reader = new InputStreamReader(is, "UTF-8");
        ...
        try {
            ...
            log.error("加载属性文件成功：{0}", fileName);
        } catch (IOException e) {
            e.printStackTrace();
            log.error("加载属性文件异常：{0}", fileName);
        } finally {
            if (is != null) {
                try {
                    is.close();
    ...
    public static String getValueByKeyWithoutLog(String key) {
        ...
        if (key != null && key.length() > 0) {
            try {
                ...
                if (!StringUtil.isEmpty(System.getProperty(key))) {
                    value = System.getProperty(key);
                    log.error("在系统属性中获取{0}的value为：{1}", key, value);
                } else {
                    value = properties.getProperty(key);
                    log.error("在配置文件中获取{0}的value为：{1}", key, value);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static String getConfigurationPath() {
        ...
        if (!path.endsWith("/")) {
            ...
            log.error("jar文件所在文件夹的路径:{0}", path);
        } else {
            log.error("classpath路径:{0}", path);
        }
