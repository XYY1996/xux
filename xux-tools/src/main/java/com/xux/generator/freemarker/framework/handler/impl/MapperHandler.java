...
public class MapperHandler extends BaseHandler<MapperInfo> {
    ...
    public void combileParams(MapperInfo info) {
        ...
        for (Map.Entry<String, String> entry : info.getEntityInfo().getPropNameColumnNames().entrySet()) {
            ...
            if (!("updated".equals(propName)) && !("updatedby".equals(propName))
                    && !("id".equals(propName) && Configuration.getString("base.database").equals(Constants.DB_MYSQL))) {
                /**
